...
```text
config/datasets.yaml          seven studies; curated sources pending
...
scripts/download/            independent download stage policy
scripts/ingest/<study>.py     seven explicit entry points, awaiting file inspection
...
The studies are Galvao, Kumar, Ayhan, Pai, Thrupp, Pappalardo and Liu. Sources,
assays and clinical mappings remain unset until curated evidence arrives.
Ingestion entry points intentionally fail without writing anything until their
actual source formats have been inspected. There is no universal loader.
...
Verified locally using the existing `scenv` Python environment (AnnData 0.11.4):
six synthetic tests passed, including dense/sparse H5AD round trips, missing
clinical metadata, duplicate-key rejection, invalid counts, existing-output
protection and raw-directory symlink protection. All seven pending ingestion
entry points refuse execution; all 27 archived file hashes match. No network
access or dependency installation was needed.
